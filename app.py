...
def generate_ai_training_data(n_samples=500):
    ...
    for _ in range(n_samples):
        ...
        # Hàm tính toán cơ bản: Nhiệt độ 27, độ ẩm 50 là lý tưởng
        ...
        # Năng suất phụ thuộc mạnh vào cả NDVI, Độ ẩm đất, Nhiệt độ và Ánh sáng
        # Cập nhật lại công thức để AI phân bổ trọng số (Feature Importance) thực tế hơn
        yield_kg = (3000 * ndvi) + (1500 * moist_factor) + (800 * temp_factor) + (500 * (sunlight/8)) + np.random.normal(0, 150)
...
with tab2:
    ...
    st.markdown("---")
    st.subheader("Kịch bản giả định (What-if Analysis)")
    ...
    baseline_inputs = pd.DataFrame([[27.0, 50.0, 8.0, 0.75]], columns=feature_names)
    ...
    with c1:
        ...
        sim_moist = st.slider("Độ ẩm đất (%)", 10.0, 90.0, 50.0, 1.0)
